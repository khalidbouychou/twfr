...
class RecommendationEngine {
  ...
  matchProducts(userData) {
    ...
    const matchedProducts = this.products.map((product) => {
      ...
      if (
        productDuration.includes(durationProfile.durationPreference) ||
      ...
      if (esgProfile.esgScore > 0) {
        esgCompatibility = 100; // Placeholder; add per-product ESG later
      }
  ...
  generateAllocation(matchedProducts) {
    ...
    if (highCompatibility.length > 0) {
      ...
      highCompatibility.forEach((product, index) => {
        allocation.push({
          ...
          value:
            index === highCompatibility.length - 1 ? 70 - highAllocation * (highCompatibility.length - 1) : highAllocation,
    ...
    if (mediumCompatibility.length > 0 && allocation.length < 3) {
      ...
      mediumCompatibility.forEach((product, index) => {
        allocation.push({
          name: product.nom_produit,
          value:
            index === mediumCompatibility.length - 1 ? 25 - mediumAllocation * (mediumCompatibility.length - 1) : mediumAllocation,
  ...
  generateRecommendations(userData, matchedProducts) {
    ...
    if (userData.savingsGoals.includes('Retraite')) {
      recommendations.push({ type: 'goal', text: 'Pensez à un PER pour optimiser votre retraite', priority: 'high' });
    }
    if (userData.savingsGoals.includes('Voyage')) {
      recommendations.push({ type: 'goal', text: 'Privilégiez des placements liquides pour vos projets de voyage', priority: 'medium' });
    }
    if (userData.savingsGoals.includes('Immobilier')) {
      recommendations.push({ type: 'goal', text: 'Envisagez un PEL pour votre projet immobilier', priority: 'high' });
    }

    if (riskProfile.riskLevel === 'Faible') {
      recommendations.push({ type: 'risk', text: 'Votre profil prudent vous permet de sécuriser votre épargne', priority: 'medium' });
    } else if (riskProfile.riskLevel === 'Élevé') {
      recommendations.push({ type: 'risk', text: "Votre tolérance au risque vous permet d'envisager des placements plus dynamiques", priority: 'medium' });
    }

    if (esgProfile.esgScore > 3) {
      recommendations.push({ type: 'esg', text: 'Nos fonds ESG correspondent à vos valeurs environnementales', priority: 'high' });
    }

    if (userData.investmentExperience.length === 0) {
      recommendations.push({ type: 'experience', text: 'Commencez par des produits simples pour vous familiariser', priority: 'high' });
    }
  ...
  generateCompleteRecommendation(userData) {
    ...
    const compatibilityScore = Math.min(
      95,
      70 + matching.matchedProducts.filter((p) => p.overallCompatibility >= 70).length * 5
    );
