  ...
  const handleConfirmInvestment = () => {
    ...
    setTimeout(() => {
      ...
      cartItems.forEach((item) => {
        ...
        // Calculate initial profit (0.1% to 0.3% based on ROI)
        const profitRate = (item.roi?.annual || 5) / 1000; // Small initial profit
        const initialProfit = Math.max(1, Math.round(amount * profitRate));
        
        // Create investment object for investmentHistory
        const newInvestment = {
        ...
        try {
          ...
          console.error('Error adding investment to context:', error);
        }
      });
